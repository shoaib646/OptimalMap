...
class TrafficFlowPredictor():
    ...
    def get_model(self, model_name):
        if self.models.get(model_name) == None:
            self.models[model_name] = load_model(
                os.path.join(os.path.dirname(__file__), 'SingleModelScats', 'model', f'{model_name}.h5'))
        return self.models.get(model_name)

    def get_scalars(self):
        _, _, _, _, _, _, _, _, self.flow_scaler, self.scats_scaler, self.days_scaler, self.times_scaler = process_data_datetime(
    ...
    def lookup_location_data(self, location: int):
        ...
        if self.location_series_data.get(location) is None:
            location_indices = [i for i in range(len(self.series_data)) if
                                self.series_data[i][self.lags] == scaled_location]
            self.location_series_data[location] = self.series_data[location_indices]

        return self.location_series_data[location]

    def get_timeseries_inputs(self, location, date, steps):
        day = date.day
        actual_time = date.hour * 60 + date.minute  # determine time in minutes
        rounded_time = 15 * math.floor(actual_time / 15)  # get current 15 minute interval
        ...
        X = np.array([day_X[time_index-i] for i in range(1,steps)])
        X = np.reshape(X, (X.shape[0], X.shape[1], 1))
        return X
